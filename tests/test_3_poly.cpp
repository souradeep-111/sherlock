...
int main(int argc, char ** argv)
{
  ...
  map<uint32_t, pair< double, double > >  property_2_interval;
  clock_t begin, end;
  ...
  double lower_limit = -0.5, upper_limit = 0.5, delta = 0.1;
  bool status;

  // Get a list of all the properties
  property_2_interval[x0] = make_pair(0.6, 0.6798577687);
  property_2_interval[x1] = make_pair(-0.5, -0.4);
  property_2_interval[x2] = make_pair(-0.5, -0.4);
  property_2_interval[x3] = make_pair(0.45, 0.5);
  property_2_interval[x4] = make_pair(-0.5, -0.45);
  ...
  for(auto each_network : network_names)
  {
    ...
    for(int dim_1 = 0; dim_1 < 10; dim_1++)
    {
      for(int dim_2 = 0; dim_2 < 10; dim_2++)
      {
        property_2_interval[x1] = make_pair(
          lower_limit + (delta * dim_1), lower_limit + (delta*(dim_1 + 1))
        );
        property_2_interval[x2] = make_pair(
          lower_limit + (delta * dim_2), lower_limit + (delta*(dim_2 + 1))
        );

        status = prove_property(property_2_interval, source_filename);
                    ...
                    string filename)
{
  ...
  uint32_t x0, x1, x2, x3, x4;
  x0 = 1; x1 = 2; x2 = 3; x3 = 4; x4 = 5;
  double property_2_low_bound;
  property_2_low_bound = 0.0;
  ...
  sherlock_parameters.grad_search_point_verbosity = true;
  // PART 1
  status = sherlock_instance.gradient_driven_target(node_index_1, region, false,
                                                  property_2_low_bound, witness);
  ...
  else
  {
    ...
    // sherlock_instance.gradient_driven_optimization(node_index_1, region, false, output_range.first,
    //                                               witness);
    // sherlock_instance.optimize_node_with_witness(node_index_1, false, region, output_range.first,
    //                                               witness);

    region_constraints output_poly;
    set < uint32_t > node_indices;
    node_indices.insert(node_index_1);
    sherlock_poly.propagate_polyhedrons(CG, region, output_poly, node_indices);
    output_poly.print();
    exit(0);

    if(output_range.first > property_2_low_bound)
      status_1 = true;
    else
    {
      status_1 = false;
  ...
  // PART 2
  status = sherlock_instance.gradient_driven_target(node_index_2, region, false,
                                                  property_2_low_bound, witness);
  ...
  {
    cout << "Attempting to prove property ....... " << endl;
   sherlock_instance.gradient_driven_optimization(node_index_3, region, false, output_range.first,
                                                    witness);
    if(output_range.first > property_2_low_bound)
      status_2 = true;
    else
  ...
  // PART 3
  status = sherlock_instance.gradient_driven_target(node_index_3, region, false,
                                                  property_2_low_bound, witness);
  ...
  // PART 4
  status = sherlock_instance.gradient_driven_target(node_index_4, region, false,
                                                  property_2_low_bound, witness);
  ...
  {
    cout << "Attempting to prove property ....... " << endl;
    sherlock_instance.gradient_driven_optimization(node_index_4, region, false, output_range.first,
                                                  witness);
    if(output_range.first > property_2_low_bound)
      status_4 = true;
    else
