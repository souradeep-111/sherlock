...
int main(int argc, char ** argv)
{
  ...
  map<uint32_t, pair< double, double > >  property_3_interval;
  clock_t begin, end;
  ...
  uint32_t x0, x1, x2, x3, x4, no_of_divs;
  x0 = 1; x1 = 2; x2 = 3; x3 = 4; x4 = 5;
  double lower_limit = 0.3, upper_limit = 0.5;
  no_of_divs = 20;
  double delta = (upper_limit - lower_limit)/(((double)no_of_divs));

  bool status;

  // Get a list of all the properties
  property_3_interval[x0] = make_pair(-0.3035311561, -0.2985528119);
  property_3_interval[x1] = make_pair(-0.0095492966, 0.0095492966);
  property_3_interval[x2] = make_pair(0.4933803236, 0.5);
  property_3_interval[x3] = make_pair(0.3, 0.5);
  property_3_interval[x4] = make_pair(0.3, 0.5);
  ...
  for(auto each_network : network_names)
  {
    ...
    for(int dim_1 = 0; dim_1 < no_of_divs; dim_1++)
    {
      ...
      {
        property_3_interval[x3] = make_pair(
          lower_limit + (delta * dim_1), lower_limit + (delta*(dim_1 + 1))
        );
        property_3_interval[x4] = make_pair(
          lower_limit + (delta * dim_2), lower_limit + (delta*(dim_2 + 1))
        );

        status = prove_property(property_3_interval, source_filename);
