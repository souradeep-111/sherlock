...
no_of_data_points = int(total_length/(no_of_inputs + 1))

x_data = np.zeros((no_of_data_points,no_of_inputs)).astype(np.float32)
y_data = np.zeros(no_of_data_points).astype(np.float32)
...
for i in range(no_of_data_points):
    ...
    pointer = pointer * (no_of_inputs + 1)
    for j in range(no_of_inputs):
        x_data[i][j] = data_stash[pointer]
        pointer += 1
    y_data[i] =  data_stash[pointer] + 100
    pointer += 1
...
y_data = y_data.reshape(no_of_data_points, 1)
...
W1 = weight_variable([no_of_inputs, hidden_units])
...
W14 = weight_variable([hidden_units, 1])
b14 = bias_variable([1])
y = tf.nn.relu(tf.matmul(r1,W14)+b14)



mean_square_error = tf.reduce_max(tf.square(y-y_))
...
for i in range(no_of_data_points):
    ...
    if(np.absolute(y - y_data[i]) > max_error):
        max_error = np.absolute(y - y_data[i])
    # print('\n')
